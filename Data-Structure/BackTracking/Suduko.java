public class Suduko {
    ...
    public static boolean sudukuSolver(int suduko[][], int row, int col) {
        // base
        if (row == 9) {
            return true;
        }
        ...
        if (suduko[row][col] != 0) {
            return sudukuSolver(suduko, nextRow, nextCol);
        }

        for (int digit = 1; digit <= 9; digit++) {
            if (isSafe(suduko, row, col, digit)) {
                suduko[row][col] = digit;
                if (sudukuSolver(suduko, nextRow, nextCol)) {
                    return true;
                }
                suduko[row][col] = 0;
            }
